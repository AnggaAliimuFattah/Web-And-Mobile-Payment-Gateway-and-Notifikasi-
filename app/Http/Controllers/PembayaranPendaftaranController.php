...
class PembayaranPendaftaranController extends Controller
{
    ...
    $tahunSekarang = Carbon::now()->year;
        return view('data-pembayaran.pendaftaran',[
    ...
    {
        $data = DB::table('pembayaran_pendaftaran')->where('siswa_uuid', $uuid)->where('kondisi','Transaksi Selesai')->get();
        $person = DB::table('data_siswa')->where('uuid', $uuid)->first();
        return view('data-pembayaran.detail-pendaftaran',[
            'data' => $data,
            'person' => $person
        ]);
    }
         ...
         public function send_bulk(Request $request)
    {
        ...
        $js = DB::table('jenis_pembayaran')->where('jenis', 'PENDAFTARAN')->first();
        
          $queryz = DB::table('data_siswa')
    ->leftJoin('pembayaran_pendaftaran', function ($join) use ($request) {
        $join->on('data_siswa.uuid', '=', 'pembayaran_pendaftaran.siswa_uuid')
            ->where('pembayaran_pendaftaran.kondisi', '=', 'Transaksi Selesai');
    ...
    ->select(
        ...
        DB::raw('COALESCE(SUM(pembayaran_pendaftaran.nominal), 0) as total_bayar'),
        DB::raw('CASE WHEN COALESCE(SUM(pembayaran_pendaftaran.nominal), 0) >=' .$js->nominal.' THEN "LUNAS" ELSE "BELUM LUNAS" END as lunas')
    )
...
🕘 Jam kerja: 08.00 â€“ 14.00 WITA
    ...
    public function ajax_data(Request $request, $uuid) {
        $pendaftaran = DB::table('pembayaran_pendaftaran')->where('uuid', $uuid)->first();
        return response()->json($pendaftaran);
    }

    
       public function tunggakan($uuid)
    {
         $jns = DB::table('jenis_pembayaran')->where('jenis', 'PENDAFTARAN')->first();

          $dt_tot = DB::table('pembayaran_pendaftaran')
          ->where('siswa_uuid', $uuid)
          ->sum('nominal');

        $totals = $dt_tot ?? 0;
        $data = DB::table('pembayaran_pendaftaran')->where('siswa_uuid', $uuid)->get();
        $person = DB::table('data_siswa')->where('uuid', $uuid)->first();
        return view('data-pembayaran.tunggakan-pendaftaran',[
            'data' => $data,
            'person' => $person,
  ...
  public function show (Request $request) {
        ...
        if($search != ''){
            ...
            $no = $start + 1;
            foreach ($query as $val) {
 $dt = DB::table('pembayaran_pendaftaran')->where('siswa_uuid', $val->uuid)->where('status', 'Lunas')->first();

          if ($dt?->nominal == null) {
            $badge = 'badge bg-danger';
            $stt = 'Belum Bayar Pendaftaran';
            $n = '
                    <a class="btn btn-sm btn-danger mt-1" href="/detail-tunggakan-pendaftaran/'.$val->uuid.'"><i class="fas fa-eye text-danger"></i> Tunggakan</a>
                    <a class="btn btn-sm btn-warning mt-1" onclick="notif(\''.$val->uuid.'\')"><i class="fas fa-eye text-warning"></i> Kirim Notifikasi</a>
                 ';
          } else {
         ...
         array(
          ...
          '<td>
         <a class="btn btn-sm btn-primary" href="/detail-pembayaran-pendaftaran/'.$val->uuid.'"><i class="fas fa-eye text-info"></i> Rincian</a>
           '.$n.'
        </td>'
...
$query = $queryz->skip($start)->take($length)->get();
          ...
          $dt = DB::table('pembayaran_pendaftaran')
         ...
         if ($totals == null || $totals == 0) { 
            ...
            $stt = 'Belum Lunas Pendaftaran Kelas' ;
            $n = '
                    <a class="btn btn-sm btn-danger mt-1" href="/detail-tunggakan-pendaftaran/'.$val->uuid.'"><i class="fas fa-eye text-danger"></i> Tunggakan</a>
                    <a class="btn btn-sm btn-primary mt-1" href="/detail-pembayaran-pendaftaran/'.$val->uuid.'"><i class="fas fa-eye text-info"></i> Rincian</a>
                    <a class="btn btn-sm btn-warning mt-1" onclick="notif(\''.$val->uuid.'\')"><i class="fas fa-eye text-warning"></i> Kirim Notifikasi</a>';
          }else if($totals >= $jns->nominal){
              $badge = 'badge bg-success';
            $stt = 'Lunas Pendaftaran Kelas' ;
            $n = '<a class="btn btn-sm btn-primary" href="/detail-pembayaran-pendaftaran/'.$val->uuid.'"><i class="fas fa-eye text-info"></i> Rincian</a>';
          }
        // $thn = $val->created_at->format('Y');
...
return response()->json($res);
     ...
     */
    public function update(Request $request, PembayaranPendaftaran $pembayaranPendaftaran)
    {
        //
    }
