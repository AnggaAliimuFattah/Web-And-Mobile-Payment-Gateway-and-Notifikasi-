...
class PembayaranUtsController extends Controller
{
    ...
    $tahunSekarang = Carbon::now()->year;
        return view('data-pembayaran.uts',[
    ...
    {
        $data = DB::table('pembayaran_uts')->where('siswa_uuid', $uuid)->get();
        $person = DB::table('data_siswa')->where('uuid', $uuid)->first();
        return view('data-pembayaran.detail-uts',[
            'data' => $data,
            'person' => $person,
            'kelas' => $kelas
        ]);
    ...
    {
        $person = DB::table('data_siswa')->where('uuid', $uuid)->first();
        return view('data-pembayaran.detail-uts2',[
             'kelas' => $kelas,
             'person' => $person,
        ]);
    }
             ...
             public function send_bulk(Request $request)
    {
        ...
        $js = DB::table('jenis_pembayaran')->where('jenis', 'UTS')->first();
        
          $queryz = DB::table('data_siswa')
    ->leftJoin('pembayaran_uts', function ($join) use ($request) {
        $join->on('data_siswa.uuid', '=', 'pembayaran_uts.siswa_uuid')
            ->where('pembayaran_uts.kondisi', '=', 'Transaksi Selesai');
    ...
    ->select(
        ...
        DB::raw('COALESCE(SUM(pembayaran_uts.nominal), 0) as total_bayar'),
        DB::raw('CASE WHEN COALESCE(SUM(pembayaran_uts.nominal), 0) >=' .$js->nominal.' THEN "LUNAS" ELSE "BELUM LUNAS" END as lunas')
    )
...
📌 Pendaftaran UTS Kelas '.$val->kelas.'
...
🕘 Jam kerja: 08.00 - 14.00 WITA
   ...
   public function tunggakan($uuid, $kelas)
    {
         $jns = DB::table('jenis_pembayaran')->where('jenis', 'uts')->first();

          $dt_tot = DB::table('pembayaran_uts')
          ->where('kelas', $kelas)
          ->where('siswa_uuid', $uuid)
          ->sum('nominal');

        $totals = $dt_tot ?? 0;
        $data = DB::table('pembayaran_uts')->where('siswa_uuid', $uuid)->where('kelas', $kelas)->get();
        $person = DB::table('data_siswa')->where('uuid', $uuid)->first();
        return view('data-pembayaran.tunggakan-uts',[
            'data' => $data,
            'person' => $person,
            'kelas' => $kelas,
    ...
    public function ajax_data(Request $request, $uuid) {
        $pendaftaran = DB::table('pembayaran_uts')->where('uuid', $uuid)->first();
    ...
    ($val->status2 == 'Pendaftar Kelas XII' && in_array($request->kelas, ['X', 'XI']))
            ...
            $stt = 'Lunas UTS Kelas '.$request->kelas.' Bulan '.$request->bulan.'' ;
            $n = '<a class="btn btn-sm btn-primary" href="/detail-pembayaran-uts2/'.$val->uuid.'/'.$request->kelas.'"><i class="fas fa-eye text-info"></i> Rincian</a>';
    }else{
         if ($totals == null || $totals == 0) { 
            $badge = 'badge bg-danger';
            $stt = 'Belum Bayar UTS Kelas '.$request->kelas.' Bulan '.$request->bulan.'';
             $n = '
                    <a class="btn btn-sm btn-danger mt-1" href="/detail-tunggakan-uts/'.$val->uuid.'/'.$request->kelas.'"><i class="fas fa-eye text-danger"></i> Tunggakan</a>
            ...
            $stt = 'Belum Lunas UTS Kelas '.$request->kelas.' Bulan '.$request->bulan.'' ;
            $n = '
                    <a class="btn btn-sm btn-danger mt-1" href="/detail-tunggakan-uts/'.$val->uuid.'/'.$request->kelas.'"><i class="fas fa-eye text-danger"></i> Tunggakan</a>
                    <a class="btn btn-sm btn-primary mt-1" href="/detail-pembayaran-uts/'.$val->uuid.'/'.$request->kelas.'"><i class="fas fa-eye text-info"></i> Rincian</a>
                    <a class="btn btn-sm btn-warning mt-1" onclick="notif(\''.$val->uuid.'\', \''.$request->kelas.'\')"><i class="fas fa-eye text-warning"></i> Kirim Notifikasi</a>';
          }else if($totals >= $jns->nominal){
              $badge = 'badge bg-success';
            $stt = 'Lunas UTS Kelas '.$request->kelas.' Bulan '.$request->bulan.'' ;
            $n = '<a class="btn btn-sm btn-primary" href="/detail-pembayaran-uts/'.$val->uuid.'/'.$request->kelas.'"><i class="fas fa-eye text-info"></i> Rincian</a>';
          }
    }
...
tunggakan pembayaran UTS Kelas '.$kelas.'
